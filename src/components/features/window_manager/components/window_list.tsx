...
const WindowList = (props: IWindowList): JSX.Element => {
	...
	const existingWindowsElements: Array<JSX.Element> = useMemo(
		() =>
			existingWindows?.map((item: iWindowItem) => {
				return (
					<WindowItem
						tabsCol={2}
						disableMark={false}
	...
	if (createWindow && inCreationId > 0) {
		...
		if (existingWindowsElements?.length > 0) {
			return <ul className="w-full list-none">{existingWindowsElements}</ul>;
		} else {
			return (
				<p>
					There are no windows in this folder. Please, create one by clicking the button
					below.
				</p>
			);
		}
