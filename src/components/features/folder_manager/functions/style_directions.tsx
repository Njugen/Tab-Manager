...
const outerStyleDirection = (type: string, show: boolean): string => {
	...
	if (type === "slide-in") {
		cssClasses = `${popup_container_transparent_bg} w-full scroll-smooth overflow-x-hidden overflow-y-scroll flex fixed top-0 left-0 justify-center items-center w-screen z-[100000] h-screen`;
	} else if (type === "popup") {
		cssClasses = `${popup_container_default} overflow-y-auto overflow-x-auto flex absolute auto top-0 left-0 justify-center items-center w-screen z-[100000] "h-screen"}`;
	}
...
const innerStyleDirection = (type: string, show: boolean): string => {
	...
	if (type === "slide-in") {
		...
		cssClasses = `w-full bg-white min-h-[200px] md:rounded-lg absolute left-0 top-0}`;
	}

	return cssClasses;
};

export { outerStyleDirection, innerStyleDirection };
