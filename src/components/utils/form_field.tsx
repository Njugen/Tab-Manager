		...
		<div className="mb-12 md:flex md:flex-row items-center">
			<div className={`sm:w-full md:w-2/5`}>
				<h4
					...
					className={`font-semibold text-lg mb-1 ${error && "text-red-500"}`}
				>
					{label}
				</h4>
				<p
					className={`text-sm leading-6 text-tbfColor-darkergrey text-start ${error && "text-red-500"}`}
				>
					{description}
				</p>
			</div>
			<div className="sm:w-full md:w-3/5 md:ml-24 my-4">{children}</div>
		</div>
	);
};
