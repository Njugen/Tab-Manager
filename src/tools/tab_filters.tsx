...
const filterSessionTabsByString = (
	arr: iCurrentSessionState,
	keyword: string
	...
	const result: Array<chrome.tabs.Tab> = collection.filter(
		(tab: chrome.tabs.Tab) =>
			tab.title && tab.title.toLowerCase().includes(keyword.toLowerCase())
	);
	return result.slice(0, 5);
};

const filterHistoryTabsByString = (
	arr: iHistoryState,
	keyword: string
): Array<chrome.history.HistoryItem> => {
	const result = arr.tabs.filter((tab: chrome.history.HistoryItem) =>
...
const filterFoldersByString = (arr: Array<iFolderItem>, keyword: string): Array<iFolderItem> => {
	const result = arr.filter((folder: iFolderItem) =>
		folder.name.toLowerCase().includes(keyword.toLowerCase())
	);
	return result.slice(0, 5);
};

export { filterSessionTabsByString, filterHistoryTabsByString, filterFoldersByString };
