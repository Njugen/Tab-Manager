...
function SidePanel(props: any): JSX.Element {
	...
	let inactiveNavButtonCSS =
		"text-gray-400 hover:text-tbfColor-lighterpurple transition ease-in-out duration-300 font-semibold";
		...
		<>
			...
			<div className={"p-4 border-b border-gray-100 sticky top-0 z-50 bg-white"}>
				...
				<nav className="flex justify-between mt-8">
					<button
						onClick={() => dispatch(setPanelView("folders"))}
						className={view === "folders" ? activeNavButtonCSS : inactiveNavButtonCSS}
					>
						Folders
					</button>
					<button
						onClick={() => dispatch(setPanelView("session"))}
						className={view === "session" ? activeNavButtonCSS : inactiveNavButtonCSS}
					>
						Current session
					</button>
					<button
						onClick={() => dispatch(setPanelView("history"))}
						className={view === "history" ? activeNavButtonCSS : inactiveNavButtonCSS}
					>
						History
					</button>
				</nav>
			...
			<footer className="shadow font-bold bg-white sticky bottom-0 px-4 py-4 border-t-2 border-t-tbfColor-lightpurple flex justify-around z-50">
				<Navlink
					...
					url="?view=main"
					isActive={false}
					onClick={() => window.open("./options.html#main", "_blank")}
				>
				...
				<Navlink
					...
					url="?view=settings"
					isActive={false}
					onClick={() => window.open("./options.html#settings", "_blank")}
				>
