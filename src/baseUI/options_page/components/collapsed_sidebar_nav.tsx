...
const CollapsedSidebarNav = (props: iSidebarNav): JSX.Element => {
    ...
    return (
      <div id="main-menu" className="flex flex-col items-center justify-center">
        <div className="">
          <div className={`my-2 border rounded-lg ${active === "main" ? "border-tbfColor-lightpurple" : "border-tbfColor-middlegrey2"}`}>
            <Navlink key="folders-nav-link" url="?view=main" isActive={active === "main" ? true : false} onClick={() => onSetActive("main")}>
              <MultipleFoldersIcon size={32} fill={active === "main" ? "rgb(109 0 194)" : "#525252"} />
            </Navlink>
          </div>
          <div className={`my-2 border rounded-lg ${active === "settings" ? "border-tbfColor-lightpurple" : "border-tbfColor-middlegrey2"}`}>
            <Navlink key="settings-nav-link" url="?view=settings" isActive={active === "settings" ? true : false} onClick={() => onSetActive("settings")}>
