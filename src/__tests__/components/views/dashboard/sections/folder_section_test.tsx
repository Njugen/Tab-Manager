	...
	describe("Test option's menu", () => {
		...
		describe("Test duplication feature", () => {
			...
			test("'duplicate' button triggers a duplication when clicked (one folder marked)", () => {
				...
				const originHeadingText = within(folders[randomIndex]).getByRole("heading", {
					level: 2
				}).innerHTML;
				...
				folders.forEach((folder) => {
					try {
						const target = within(folder).getByRole("heading", {
							level: 2
						}).innerHTML;
		...
		describe("Test folder creation process", () => {
			...
			test("Saving the folder manager once correctly filled, will add the new folder to list", () => {
				...
				const newWindowButton = within(folderManager).getByText("New window", {
					selector: "button"
				});
				...
				const createButton = within(folderManager).getByText("Create", {
					selector: "button"
				});
				...
				folders.forEach((folder) => {
					try {
						...
						const targetHeading = within(folder).getByText(randomName, {
							selector: "h2"
						});
		...
		describe("Test merge process", () => {
			...
			test("Creating folder by merging multiple folders works", () => {
				...
				const createButton = within(folderManager).getByText("Save", {
					selector: "button"
				});
		...
		describe("Test folder removal", () => {
			test.each([
				...
				[
					"Folder is removed when proceeding through its warning message (if set in plugin's settings)",
					true,
					false
				]
				...
				const targetHeading = within(target).getByRole("heading", {
					level: 2
				});
				...
				currentFolders.forEach((folder) => {
					try {
						within(folder).getByText(headingText, {
							selector: "h2"
						});
			...
			test("Clicking trash button in option bar removes marked folders", () => {
				...
				currentFolders.forEach((folder, i) => {
					try {
						const folderName = within(folder).getByText(headingsText[i], {
							selector: "h2"
						});
		...
		describe("Test sort", () => {
			test.each([
				...
				folders.forEach((folder) => {
					const name = within(folder).getByRole("heading", {
						level: 2
					});
				...
				const targetOption = within(optionsList).getByText(option, {
					selector: "button"
				});
		...
		describe("Test folder edit", () => {
			...
			test("Clicking the cog icon renders the folder manager prefilled with folder name", () => {
				...
				const folderHeading = within(target).getByRole("heading", {
					level: 2
				});
