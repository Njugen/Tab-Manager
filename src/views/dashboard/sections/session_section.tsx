...
const SessionSection = (props: any): JSX.Element => {
	...
	useEffect(() => {
		...
		chrome.windows.onCreated.addListener(getAllWindows);
		chrome.windows.onRemoved.addListener(getAllWindows);
		chrome.tabs.onCreated.addListener(getAllWindows);
		chrome.tabs.onRemoved.addListener(getAllWindows);
		chrome.tabs.onDetached.addListener(getAllWindows);
		chrome.tabs.onMoved.addListener(getAllWindows);
		chrome.tabs.onReplaced.addListener(getAllWindows);
		chrome.tabs.onUpdated.addListener(getAllWindows);

		return () => {
			chrome.windows.onCreated.removeListener(getAllWindows);
			chrome.windows.onRemoved.removeListener(getAllWindows);
			chrome.tabs.onCreated.removeListener(getAllWindows);
			chrome.tabs.onRemoved.removeListener(getAllWindows);
			chrome.tabs.onDetached.removeListener(getAllWindows);
			chrome.tabs.onMoved.removeListener(getAllWindows);
			chrome.tabs.onReplaced.removeListener(getAllWindows);
			chrome.tabs.onUpdated.removeListener(getAllWindows);
		};
	}, []);

	const getAllWindows = useCallback((): void => {
	...
	const showFolderManager = (): JSX.Element => {
		...
		if (createFolder === true) {
			const presetWindows: Array<iWindowItem> = sessionSectionState.windows.map(
				(window: chrome.windows.Window) => {
					if (window.tabs) {
		...
		<>
			{windowIdWarning >= 0 && (
				<PopupMessage
					title="Warning"
					text={
						"This is the only window currently open. Closing it will close your browser, do you wish to proceed?"
					}
					primaryButton={{
