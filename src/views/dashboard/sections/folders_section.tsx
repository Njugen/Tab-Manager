...
const FoldersSection = (props: any): JSX.Element => {
		...
		if (createFolder) {
			...
			if (targetFolder.length > 0) {
				render = (
					<FolderManager
						type="slide-in"
						title={`Edit folder ${targetFolder[0].name}`}
						folder={input}
						onClose={handleCloseFolderManager}
					/>
				);
	...
	const folderList = useMemo(() => {
		...
		const columnsRender: Array<JSX.Element> = colsList.map((col, i: number) => {
			return <div key={`column-key-${i}`}>{col}</div>;
		});
	...
	const handlePrepareDuplication = (): void => {
		...
		if (markedFoldersId.length > 0) {
			chrome.storage.local.get("duplicationWarningValue", (data) => {
				...
				if (
					data.duplicationWarningValue !== -1 &&
					data.duplicationWarningValue <= foldersSectionState.markedFoldersId.length
				) {
				...
				<>
					{showPerformanceWarning && (
						<PopupMessage
							...
							primaryButton={{
								...
								callback: () => {
									if (windowsPayload) {
										handleLaunchFolder(windowsPayload, folderLaunchBehavior);
									}
									setShowPerformanceWarning(false);
								}
							}}
					...
					{showDeleteWarning === true && (
						<PopupMessage
							...
							primaryButton={{
								text: "Yes, remove these folders",
								callback: () => handleDeleteFolders()
							}}
							secondaryButton={{
								text: "No, don't remove",
								callback: () => setShowDeleteWarning(false)
							}}
						/>
					)}
					{showFolderManager()}
					...
					>
						<>
							...
							{
								<ul
									className={`list-none ${foldersSectionState.viewMode === "list" ? "mx-auto mt-12" : `grid xl:grid-cols-2 2xl:grid-cols-3 3xl:grid-cols-4 grid-flow-dense gap-x-4 gap-y-0 mt-8`}`}
								>
									{folderList}
								</ul>
							}
